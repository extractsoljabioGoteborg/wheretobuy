...
'<input type="text" id="openai-chat-input" placeholder='+'"' + 'Skriv din fråga...' +'"'+ '>' +
...
addChatbotMessage("Hallå! Jag är chatboten. Hur kan jag hjälpa dig? (det första meddelandet kan ta några sekunder)");
...
jQuery(document).ready(function($) {
    ...
    async function handleQuestion(questionElement, answerElement) {
        ...
        $.get('https://raw.githubusercontent.com/medanticancerbelievescotland/knowledge/main/knowledge_swe.txt')
            .then(function(data) {
                var knowledge = data;    
                    ...
                    $.ajax({
                        ...
                        beforeSend: function(xhr) {
                            ...
                            $(answerElement).append('<div class="loading">Hämtar svar...</div>');
                        },
                        success: function(response) {
                            $('.loading').remove();
                            if (response.choices && response.choices[0] && response.choices[0].text) {
                                $(answerElement).append('<div class="question-answer"><strong>Fråga:</strong> ' + question + '<br><strong>Svar:</strong> ' + response.choices[0].text + '</div><hr>');
                                ...
                                $(answerElement).append('<div class="error"><strong>Error:</strong> Det gick inte att hämta svar.</div><hr>');
                            }
                        },
                        error: function() {
                            $('.loading').remove();
